...
// Handle for untyped pointers (SPV_KHR_untyped_pointers). Key by address space only.
inline IRHandle irhandle_untyped_pointer(unsigned AddressSpace) {
  return std::make_tuple(nullptr, AddressSpace, SpecialTypeKind::STK_UntypedPointer);
}

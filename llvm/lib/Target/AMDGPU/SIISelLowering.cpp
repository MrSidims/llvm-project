...
SDValue SITargetLowering::packBytesToI32(SelectionDAG &DAG, const SDLoc &SL,
  ...
  // 2-byte slice. Bitcast a vector to the same-width integer, optionally
  // shift down to the requested slice, truncate to i16, and zext to i32.
...
SITargetLowering::LowerCONVERT_FROM_ARBITRARY_FP(SDValue Op,
                                                 SelectionDAG &DAG) const {
  // Only different OCP FP8 formats (E4M3FN, E5M2). FNUZ formats (used by
  // gfx942) go through the generic bit-twiddling expansion.
  bool IsBF8 = false;

...
bool SelectionDAGLegalize::ExpandNode(SDNode *Node) {
  ...
  case ISD::CONVERT_TO_ARBITRARY_FP: {
    ...
    // For IEEE-like formats: DstExpMax - 1 (since DstExpMax means Inf/NaN).
    // For FiniteOnly/NanOnly formats: DstExpMax (all exponent values are finite
    // normals, except possibly a single NaN encoding).
    ...
    // For NanOnly formats (E4M3FN): the max exponent field for finite values
    // is DstExpMax, but the encoding with exp=DstExpMax and mant=all-ones is
    // NaN. So DstExpMaxNormal = DstExpMax, but max mantissa at that exponent
    // is DstMantMask - 1 (if NanEnc == AllOnes) to avoid the NaN encoding.
    ...
    // Work in the source integer type (e.g. i32 for f32).
    ...
    // Bitcast source float to integer.
    ...
    // Source is IEEE: NaN = exp_all_ones && mant != 0.
    SDValue IsNaN =
    ...
    // Normalized mantissa (shift left to place leading 1 at bit SrcMant, then
    // mask off the implicit 1).
    SDValue NormMant =
        DAG.getNode(ISD::AND, dl, IntVT,
                    DAG.getNode(ISD::SHL, dl, IntVT, SrcMantField, NormShift),
                    DAG.getConstant(SrcMantMask, dl, IntVT));

    // effective_exp = 1 - NormShift (biased)
    SDValue DenormSrcExp =
        DAG.getNode(ISD::SUB, dl, IntVT, One, NormShift);

    // Select between normal and denorm source: pick the effective exponent
    // and mantissa.
    SDValue EffSrcExp =
    ...
    if (SrcMant > DstMant) {
      ...
      // Truncated mantissa (top DstMant bits of source mantissa).
      TruncMant =
      ...
      // Sticky bits: OR of all bits below the round bit.
      SDValue StickyBits;
      if (Shift >= 2) {
      ...
      // LSB of truncated mantissa (for ties-to-even).
      ...
      if (RoundMode == RoundingMode::NearestTiesToEven) {
        ...
        RoundUp = DAG.getNode(ISD::AND, dl, IntVT, RoundBit, StickyOrLSB);
      } else if (RoundMode == RoundingMode::TowardZero) {
        RoundUp = Zero;
      } else if (RoundMode == RoundingMode::TowardPositive) {
        // Round up if positive and any truncated bits are set.
        SDValue AnyTruncBits = DAG.getNode(ISD::OR, dl, IntVT, RoundBit,
                                           StickyBits);
        ...
        RoundUp = DAG.getNode(ISD::ZERO_EXTEND, dl, IntVT, DoRound);
      } else if (RoundMode == RoundingMode::TowardNegative) {
        // Round up (toward -inf) if negative and any truncated bits are set.
        SDValue AnyTruncBits = DAG.getNode(ISD::OR, dl, IntVT, RoundBit,
                                           StickyBits);
        SDValue HasTruncBits =
            DAG.getSetCC(dl, SetCCVT, AnyTruncBits, Zero, ISD::SETNE);
        ...
        RoundUp = DAG.getNode(ISD::ZERO_EXTEND, dl, IntVT, DoRound);
      } else if (RoundMode == RoundingMode::NearestTiesToAway) {
        // Round up if round bit is set.
        RoundUp = RoundBit;
      } else {
        llvm_unreachable("Unsupported rounding mode");
      }
    } else {
      // DstMant >= SrcMant: no rounding needed, just shift left.
      SDValue MantShift =
          DAG.getShiftAmountConstant(DstMant - SrcMant, IntVT, dl);
    ...
    SDValue AdjExp = DAG.getNode(
        ...
        DAG.getNode(ISD::ZERO_EXTEND, dl, IntVT,
                    DAG.getSetCC(dl, SetCCVT, RoundedMant,
                                 DAG.getConstant(DstMantMask, dl, IntVT),
                                 ISD::SETGT)));
    ...
    SDValue DenormResult;
    {
      ...
      // full_src_mant = (1 << SrcMant) | EffSrcMant  (add implicit 1)
      ...
      SDValue DenormRoundUp;
      {
        ...
        // shift nodes with invalid (wrapped-around) shift amounts.
        SDValue SafeShift =
        ...
        if (RoundMode == RoundingMode::NearestTiesToEven) {
          SDValue StickyOrLSB =
              DAG.getNode(ISD::OR, dl, IntVT, HasSticky, DenormLSB);
          DenormRoundUp =
              DAG.getNode(ISD::AND, dl, IntVT, DenormRoundBit, StickyOrLSB);
        } else if (RoundMode == RoundingMode::TowardZero) {
          DenormRoundUp = Zero;
        } else if (RoundMode == RoundingMode::TowardPositive) {
          SDValue AnyTruncBits =
              DAG.getNode(ISD::OR, dl, IntVT, DenormRoundBit, HasSticky);
          SDValue HasTruncBits =
              DAG.getSetCC(dl, SetCCVT, AnyTruncBits, Zero, ISD::SETNE);
          SDValue IsPositive =
              DAG.getSetCC(dl, SetCCVT, SignBit, Zero, ISD::SETEQ);
          SDValue DoRound =
              DAG.getNode(ISD::AND, dl, SetCCVT, HasTruncBits, IsPositive);
          DenormRoundUp = DAG.getNode(ISD::ZERO_EXTEND, dl, IntVT, DoRound);
        } else if (RoundMode == RoundingMode::TowardNegative) {
          SDValue AnyTruncBits =
              DAG.getNode(ISD::OR, dl, IntVT, DenormRoundBit, HasSticky);
          SDValue HasTruncBits =
      ...
      // If rounding caused overflow into the normal range (mant > DstMantMask),
      // then we get the smallest normal number.
      ...
      SDValue SignForDenorm = DAG.getNode(
          ISD::SHL, dl, IntVT, SignBit,
          DAG.getShiftAmountConstant(DstBits - 1, IntVT, dl));
      DenormResult = DAG.getNode(
          ISD::OR, dl, IntVT,
          DAG.getNode(ISD::OR, dl, IntVT, SignForDenorm, DenormExpShifted),
          DenormFinalMant);

      // If the value is too small for even a denorm (all mantissa bits
    ...
    if (Saturate) {
      // Clamp to max finite value: sign | (DstExpMaxNormal << DstMant) |
      // DstMaxMantAtMaxExp
      uint64_t MaxFinite =
      ...
      // Produce infinity: sign | (DstExpMax << DstMant)
      uint64_t InfBits = (uint64_t)DstExpMax << DstMant;
      OverflowResult =
          DAG.getNode(ISD::OR, dl, IntVT, SignShifted,
                      DAG.getConstant(InfBits, dl, IntVT));
    } else {
      // No Inf in format and not saturating -> poison.
    ...
    SDValue NaNResult;
    if (DstNFBehavior == fltNonfiniteBehavior::IEEE754) {
      // Produce canonical NaN: sign=0, exp=all-ones, quiet NaN bit set.
      const uint64_t QNaNBit = (DstMant > 0) ? (1ULL << (DstMant - 1)) : 0;
      NaNResult =
